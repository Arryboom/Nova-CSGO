...
struct Vector {
    ...
    auto toAngle() const noexcept {
        return Vector{ 
            utils::RAD2DEG(std::atan2(-z, std::hypot(x, y))),           
            utils::RAD2DEG(std::atan2(y, x)),
            0.0f 
        };
    }
    ...
    static auto fromAngle(const Vector& angle) noexcept {
        return Vector{ 
            std::cos(utils::DEG2RAD(angle.x)) * std::cos(utils::DEG2RAD(angle.y)),
            std::cos(utils::DEG2RAD(angle.x)) * std::sin(utils::DEG2RAD(angle.y)),
            -std::sin(utils::DEG2RAD(angle.x))
        };
    }
